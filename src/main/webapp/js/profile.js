	...
	if(e)
		if(e = jQuery("#" + e), e.size() > 0) {
			var h = e.offset(),
				...
				m = h.top - r - d,
				y = h.left - (c - _) / 2;
			s - _ - y + d < c ? (y = s - c - d, h.top - jQuery(document).scrollTop() < r ? (m = h.top + f + d, jQuery(".ui_coast").css({
				width: t,
				left: y,
				top: m
				...
				left: y,
				top: m
			}).show()) : h.top - jQuery(document).scrollTop() < r ? (m = h.top + f + d, y < 10 && (y = 10), jQuery(".ui_coast").css({
				width: t,
				left: y,
				top: m
...
var tips = function() {
		...
		t.prototype.confirm = function(t, e, i) {
			...
			p + l > h && (p = h - l - 20), p <= 0 && (p = 20), jQuery(u).hide(), jQuery(u).css({
				left: p,
				top: c + r
			}).animate({
				height: "show",
