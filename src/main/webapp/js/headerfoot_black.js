$(function() {
	function e(e) {
		...
		if(/static.qyer.com/.test(r) === !0) {
			...
			n = $.extend({}, n, {
				__qFED__: e.__qFED__
			}), e.__qFED__ && e.__qFED__.id && (l = e.__qFED__.id, e.type = "GET"), a = "http://fe.2b6.me:3000/service/api/" + l, o = "jsonp", api = null
		}
		var c = $.ajax({
			type: e.type || "POST",
			url: a,
	...
	function t(e) {
		this.opts = $.extend({
			box: "#js_qyer_header_userStatus"
		}, e), this.$userStatusBox = $(this.opts.box), this.ajaxUrls = {
	...
	function s(e) {
		this.opts = $.extend({
			box: "#siteSearch"
		}, e), this.$searchBox = $(this.opts.box), this.$input = this.$searchBox.find(".txt"), this.$form = this.$searchBox.find("form"), this.searchUrl = this.$form.attr("action"), this.ajaxUrls = {
	...
	t.prototype = {
			init: function() {
				window.QYER = window.QYER || {
					uid: 0
				}, this.getUserStatus(), this.bindEvent()
			},
			...
			getUserMsgCount: function() {
				...
				var s = function() {
					var s = !!i,
						r = 10 * Math.random() < 7,
						a = !t;
					s || r || a || (e.hasNewMessage = !0, $.getJSON(e.ajaxUrls.msgCount, {
						timer: (new Date).getTime()
					}).done(function(t) {
			...
			getUserMsgInfo: function() {
				var e = this;
				this.hasNewMessage = !1, $.getJSON(this.ajaxUrls.msgInfo, {
					timer: (new Date).getTime()
				}).done(function(t) {
		...
		function(e, t) {
			...
			var r = !(/Android|iPhone|SymbianOS|Windows Phone/gi.test(navigator.userAgent) && /^(https?:)?\/\/m\.qyer\.com\/?.*$/.test(window.location.href)),
				a = {
					...
					createRquest: function(e) {
						var t = e.getAttribute("data-countries"),
							i = e.getAttribute("data-areas"),
							s = a.ad_request,
							r = e.id.replace(/zoneid-/, "");
					...
					getClassNames: function(t, s) {
						function r(e, t) {
							for(var i = e.className.split(/\s+/), s = 0; s < i.length; s++)
								if(i[s] == t) return !0;
							return !1
						}
			...
			bindEvent: function() {
				...
				this.$input.on("focus", function() {
					...
					var t = $.trim(e.$input.val()),
						i = e.searchUrl + "?wd=" + window.encodeURIComponent(t);
			...
			createHistory: function() {
				...
				var e = this,
					t = ['<div class="q-layer q-layer-sitesearch-history">', "<ul>", "</ul>", '<div class="history-clear">', '<a href="javascript:;">清空历史记录</a>', "</div>", "</div>"];
			...
			setStorage: function(e, t) {
				if(!window.localStorage) return !1;
				for(var i = -1, s = 0; s < this._history.length; s++)
					if(this._history[s].key == e) {
						i = s;
						break
					}
			...
			switchUpDownInHistory: function(e) {
				var t = this.$history.find("li"),
					i = this.$history.find("li.current"),
					s = null;
			...
			switchUpDownInAutoComplete: function(e) {
				var t = this.$autocomplete.find("li"),
			...
			setSource: function() {
				if(this.getSource(), !this.source) {
					var e = ".+",
						t = new RegExp("^http://(" + e + ").(qyer|go2eu).com", "i");
